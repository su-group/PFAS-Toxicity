...
# --- 配置 ---
# DATA_CONFIG = {
#     "data_dir": "F:\GNN-pro\data\spilt_1",
#     "csv_file": r"F:\GNN-pro\data\spilt_1\train",
#     "smiles_col": "SMILES",
...
TRANSFER_CONFIG = {
    ...
    'new_data_config': {
        'data_dir': r'F:\GNN-pro\data\cf2_data_start', # 新数据存放目录
        'train_csv': 'train_augmented.csv',      # 已划分好的训练集
        'val_csv': 'val_filled.csv',          # 已划分好的验证集
        'test_csv': 'test_filled.csv',        # 已划分好的测试集
        'smiles_col': 'SMI',        # SMILES列名（请根据你的文件调整）
        'label_col': 'NeglogLD50',   # 新任务的标签列名
        'batch_size': 32,              # 批次大小
    },
    'new_model_config': {
        'params': {
...
def load_data(config):
    """
    根据配置加载已划分好的训练、验证、测试数据集，并清除缓存以确保特征维度一致
    """
    import shutil

    data_dir = config['data_dir']
    train_path = os.path.join(data_dir, config['train_csv'])
    val_path = os.path.join(data_dir, config['val_csv'])
    test_path = os.path.join(data_dir, config['test_csv'])
...
def transfer_learn():
    ...
    # 加载数据
    print("加载新任务数据...")
    train_loader, val_loader, test_loader = load_data(new_data_config)
